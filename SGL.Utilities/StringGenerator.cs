...
namespace SGL.Analytics.Utilities {
	public static class StringGenerator {
		...
		public static string GenerateRandomString(int length) {
			return new string(Enumerable.Range(0, 256).Select(_ => charactersWithSpace[rnd.Next(charactersWithSpace.Length)]).ToArray());
		}
		public static string GenerateRandomWord(int length) {
			return new string(Enumerable.Range(0, 256).Select(_ => wordCharacters[rnd.Next(wordCharacters.Length)]).ToArray());
		}
	}
}
