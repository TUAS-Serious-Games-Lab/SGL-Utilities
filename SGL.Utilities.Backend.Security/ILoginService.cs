...
namespace SGL.Utilities.Backend.Security {
	...
	/// <summary>
	/// Provides common interface elements for <see cref="ILoginService"/> and <see cref="ILoginAuthenticationService"/>.
	/// </summary>
	public interface ILoginServiceBase {
		...
		/// <returns>A handle to pass to <c>LoginAsync</c> in <see cref="ILoginService"/> or <see cref="ILoginAuthenticationService"/>
		/// to cover exernal opterations in the fixed-time delay upon failure.</returns>
		/// <remarks>
	...
	/// This delay timer is represented by an <see cref="ILoginServiceBase.IDelayHandle"/> and started using <see cref="ILoginServiceBase.StartFixedFailureDelay(CancellationToken)"/>.
	///
	/// The interface specifies four overloads of <c>LoginAsync</c> to make both, the cancellation token and a pre-started <see cref="ILoginServiceBase.IDelayHandle"/> independently optional.
	/// However, implementations only need to provide the fully specified overload and <see cref="ILoginServiceBase.StartFixedFailureDelay(CancellationToken)"/>.
	///
	/// The user and secret management is wired up using delegates to minimize coupling to specific user management environments.
	/// </summary>
	/// <remarks>
	/// This is the older version of <see cref="ILoginAuthenticationService"/> and returns plain bearer token values.
	/// Usually, <see cref="ILoginAuthenticationService"/> is preferable as it returns a full <see cref="AuthorizationData"/> upon success.
	/// </remarks>
	public interface ILoginService : ILoginServiceBase {
		...
		/// <summary>
		/// An overload of <c>LoginAsync</c> where no <see cref="ILoginServiceBase.IDelayHandle"/> is given and <see cref="ILoginServiceBase.StartFixedFailureDelay(CancellationToken)"/> is called at the start instead.
		...
		/// <summary>
		/// An overload of <c>LoginAsync</c> where neither a <see cref="CancellationToken"/> nor an <see cref="ILoginServiceBase.IDelayHandle"/> are given.
		/// The operation therefore automatically calls <see cref="ILoginServiceBase.StartFixedFailureDelay(CancellationToken)"/> at the beginning and can not be cancelled.
		/// See the full overload for further details.
		/// </summary>
		Task<string?> LoginAsync<TUserId, TUser>(
			TUserId userId, string providedPlainSecret,
	...
	/// <summary>
	/// Specifies the interface for a login service, that authenicates users using credentials in the form of a userid and a secret string.
	/// If the authentication is successful, it issues an authorization token for the user and returns it as <see cref="AuthorizationData"/>.
	/// Otherwise, implementations should wait for a fixed delay timer, started at the beginning of the authentication, in all failure paths to prevent timing attacks to reveal if the user id or the secret was incorrect.
	/// This delay timer is represented by an <see cref="ILoginServiceBase.IDelayHandle"/> and started using <see cref="ILoginServiceBase.StartFixedFailureDelay(CancellationToken)"/>.
	///
	/// The interface specifies four overloads of <c>LoginAsync</c> to make both, the cancellation token and a pre-started <see cref="ILoginServiceBase.IDelayHandle"/> independently optional.
	...
	/// </summary>
	/// <remarks>
	/// This is improved version of <see cref="ILoginService"/> that returns <see cref="AuthorizationData"/> instead of plain bearer token values.
	/// </remarks>
	public interface ILoginAuthenticationService : ILoginServiceBase {
		/// <summary>
		/// Asynchronously perform a login operation with the given credentials, using the given delegates to access the user management.
		/// </summary>
		/// <typeparam name="TUserId">The data type used for the user id (e.g. <see cref="string"/>, <see cref="Guid"/>, <see cref="int"/>).</typeparam>
		...
		/// If the login failed it has a <see langword="null"/> result after the fixed delay time has expired.
		/// </returns>
		Task<AuthorizationData?> LoginAsync<TUserId, TUser>(
			TUserId userId, string providedPlainSecret,
			Func<TUserId, Task<TUser?>> lookupUserAsync,
			Func<TUser, string> getHashedSecret,
			Func<TUser, string, Task> updateHashedSecretAsync,
